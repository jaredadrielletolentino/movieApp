...
export default function MovieDetails() {
  ...
  const fetchMovieAndComments = () => {
    ...
    fetch(`https://movieapp-9vzc.onrender.com/movies/getMovie/${id}`)
      .then(res => res.json())
      .then(data => {
        if (data._id) {
          setMovie(data);
          if (user.id) {
            ...
            if (token) {
              fetch(`https://movieapp-9vzc.onrender.com/movies/getComments/${id}`, {
                headers: {
  ...
  const handleAddComment = (e) => {
    ...
    fetch(`https://movieapp-9vzc.onrender.com/movies/addComment/${id}`, {
      method: 'PATCH',
      headers: {
  ...
  const handleDeleteComment = (commentId) => {
    if (window.confirm('Are you sure you want to delete this comment?')) {
      ...
      fetch(`https://movieapp-9vzc.onrender.com/movies/deleteComment/${id}/${commentId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
